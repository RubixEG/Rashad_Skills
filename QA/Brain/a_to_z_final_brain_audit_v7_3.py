...
def main():
    ...
    # A — Authority
    add('A','canonical_skill_version_7_3',ver['canonical_skill_version']==osst['canonical_skill_version']==bm['bound_skill_version']==am['version']=='7.3.0',[ver['canonical_skill_version'],osst['canonical_skill_version'],bm['bound_skill_version'],am['version']])
    add('A','brain_version_3_5',ver['brain_runtime_version']==osst['brain_runtime_version']==bm['brain_version']==am['brain_runtime_version']=='3.5.0')
    add('A','global_authorities_exist',all((S/p).is_file() for p in am['global_authorities']))
    add('A','current_cert_harness_v73',am.get('certification_harness')=='tests/skill_certification/verify_skill_v7_3.py')
    # B — Brain
    add('B','brain_manifest_bound_to_v73',bm.get('bound_skill_version')=='7.3.0')
    ...
    add('S','skill_redteam_current_declared',cr.get('current_red_team')=='tests/skill_certification/red_team_skill_v7_3.py')
    # T — Technical/AI
    ...
    # V — Versions/status
    add('V','skill_status_v73',cs.get('skill_version')=='7.3.0')
    add('V','cert_requirements_v73',cr.get('version')=='7.3.0')
    add('V','artifact_registry_v40',art.get('artifact_brain_version')=='4.0.0')
    add('V','brain_manifest_v35',bm.get('brain_version')=='3.5.0')
    current_docs={
    ...
    for name,fp in current_docs.items():
        tx=fp.read_text(encoding='utf-8',errors='ignore')
        if ('v7.3' not in tx and '7.3.0' not in tx and name not in ['QA/Runtime/VERSION.md','QA/Runtime/README.md','Rashad/Docs/HANDOFF.md']) or re.search(r'canonical[^\n]{0,80}v7\.1|bound skill:[^\n]*v7\.1|current v7\.1 startup route|Rashad OS v7\.1 \/ Brain v3\.2',tx,re.I): stale_docs.append(name)
    add('V','current_facing_docs_are_v73_not_stale_v71',not stale_docs,stale_docs)
    pending_current=[]
    ...
    add('V','current_status_layer_has_no_stale_v72_finalization',not pending_current,pending_current)
    # W — Workflow product
    ...
    add('Y','skill_manifest_v73_exact_all_current_bytes',(S/'MANIFEST.md').read_text(encoding='utf-8').startswith('# Rashad Proposal OS v7.3.0') and manifest_map==actual_skill,{'manifest':len(manifest_map),'actual':len(actual_skill),'missing':sorted(set(actual_skill)-set(manifest_map))[:5],'extra':sorted(set(manifest_map)-set(actual_skill))[:5]})
    ...
    ir73=J(Q/'Certification/INCIDENT_REGRESSION_V7_3.json') if (Q/'Certification/INCIDENT_REGRESSION_V7_3.json').exists() else {}
    ...
    add('Z','v73_incident_registry_pass',ir73.get('status')=='PASS' and ir73.get('incident_count')==16,{'status':ir73.get('status'),'incidents':ir73.get('incident_count')})

    # v7.3 production intelligence / engagement acceptance closure
    rem=J(Q/'Certification/REMEDIATION_MATRIX_V7_3.json') if (Q/'Certification/REMEDIATION_MATRIX_V7_3.json').exists() else {}
    eng=J(Q/'Certification/ENGAGEMENT_ARTIFACT_ACCEPTANCE_V7_3.json') if (Q/'Certification/ENGAGEMENT_ARTIFACT_ACCEPTANCE_V7_3.json').exists() else {}
    vp=J(Q/'Certification/VISUAL_PRODUCTION_V7_3_RESULTS.json') if (Q/'Certification/VISUAL_PRODUCTION_V7_3_RESULTS.json').exists() else {}
    gd=J(Q/'Certification/GOVERNED_PRODUCTION_DELIVERY_V7_3.json') if (Q/'Certification/GOVERNED_PRODUCTION_DELIVERY_V7_3.json').exists() else {}
    add('Z','v73_remediation_matrix_zero_open',rem.get('status')=='PASS' and rem.get('open_count',rem.get('open'))==0 and rem.get('open_p0_p1')==0,rem)
    add('Z','v73_visual_production_certified',vp.get('status')=='PASS',vp.get('status'))
    add('Z','v73_governed_positive_path_satisfiable',gd.get('status')=='PASS',gd.get('status'))
    erows=eng.get('artifacts',eng.get('results',[]))
    add('Z','v73_actual_engagement_files_measured',eng.get('status')=='PASS' and len(erows)>=2 and all(x.get('sha256') and x.get('acceptance_status')=='BLOCKED' for x in erows),erows)
    add('Z','v73_page_composition_spec_runtime',(B/'runtime/brain/composition_spec.py').exists() and 'RASHAD_PAGE_COMPOSITION_SPEC_V1' in T(B/'runtime/brain/composition_spec.py'))
    add('Z','v73_production_composer_runtime',(B/'runtime/brain/production/composer.py').exists() and 'data-region-id' in T(B/'runtime/brain/production/composer.py'))
    add('Z','v73_format_neutral_inspector',(B/'runtime/brain/product_geometry.py').exists() and 'def pdf_model' in T(B/'runtime/brain/product_geometry.py') and 'NO_REGISTERED_INSPECTOR_FOR_FORMAT' in T(B/'runtime/brain/product_geometry.py'))
    ...
    out={'suite':'Rashad v7.3 Final A-to-Z Brain/Architecture/Workflow/QA Audit','status':'PASS' if passed==total else 'FAIL','passed':passed,'total':total,'categories':categories}
    dest=Q/'Certification/V7_3_FINAL_A_TO_Z_AUDIT.json'; dest.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
