...
def main():
    ...
    # A — Authority
    add('A','canonical_skill_version_7_2',ver['canonical_skill_version']==osst['canonical_skill_version']==bm['bound_skill_version']==am['version']=='7.2.0',[ver['canonical_skill_version'],osst['canonical_skill_version'],bm['bound_skill_version'],am['version']])
    ...
    add('A','current_cert_harness_v72',am.get('certification_harness')=='tests/skill_certification/verify_skill_v7_2.py')
    # B — Brain
    add('B','brain_manifest_bound_to_v72',bm.get('bound_skill_version')=='7.2.0')
    ...
    rex=J(S/'RETRIEVAL_EXCLUSION_REGISTRY.json'); add('L','retrieval_quarantine_v72',rex.get('version')=='7.2.0')
    ...
    add('S','skill_redteam_current_declared',cr.get('current_red_team')=='tests/skill_certification/red_team_skill_v7_2.py')
    # T — Technical/AI
    ...
    # V — Versions/status
    add('V','skill_status_v72',cs.get('skill_version')=='7.2.0')
    add('V','cert_requirements_v72',cr.get('version')=='7.2.0')
    ...
    add('Y','skill_manifest_v72_exact_all_current_bytes',(S/'MANIFEST.md').read_text(encoding='utf-8').startswith('# Rashad Proposal OS v7.2.0') and manifest_map==actual_skill,{'manifest':len(manifest_map),'actual':len(actual_skill),'missing':sorted(set(actual_skill)-set(manifest_map))[:5],'extra':sorted(set(manifest_map)-set(actual_skill))[:5]})
    ...
    out={'suite':'Rashad v7.2 Final A-to-Z Brain/Architecture/Workflow/QA Audit','status':'PASS' if passed==total else 'FAIL','passed':passed,'total':total,'categories':categories}
    dest=Q/'Certification/V7_2_FINAL_A_TO_Z_AUDIT.json'; dest.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
