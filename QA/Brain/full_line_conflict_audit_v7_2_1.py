...
def main():
    ...
    expected={'skill':'7.2.1','brain':'3.4.0','artifact':'3.3.0','qa':'4.3','qabrain':'1.3.0'}
    ...
    if m.get('certification_harness')!='tests/skill_certification/verify_skill_v7_2_1.py': findings.append({'severity':'P0','kind':'STALE_CURRENT_CERT_HARNESS','value':m.get('certification_harness')})
    if 'tests/skill_certification/red_team_skill_v7_2_1.py' not in m.get('certification_harnesses',[]): findings.append({'severity':'P0','kind':'STALE_CURRENT_REDTEAM_HARNESS'})
    ...
    for idx in [ROOT/'Rashad/Certification/CURRENT_CERTIFICATION_INDEX.json',ROOT/'QA/Certification/CURRENT_CERTIFICATION_INDEX.json']:
        if idx.exists():
            x=J(idx)
            if x.get('canonical_skill_version') not in (None,'7.2.1') or str(x.get('status','')).startswith('CURRENT_V7_1'):
    ...
    out={'suite':'Rashad v7.2.1 Full Line Conflict Audit','status':'PASS' if not p0 and not p1 else 'FAIL','stats':stats,'manifest_authorities':len(globals),'p0_count':len(p0),'p1_count':len(p1),'findings':findings}
    dest=QA/'Certification/FULL_LINE_CONFLICT_AUDIT_V7_2_1.json'; dest.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
