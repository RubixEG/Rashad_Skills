...
add('A01_skill_version_split_brain_blocked',ver.get('canonical_skill_version')==osj.get('canonical_skill_version')==bm.get('bound_skill_version')==m.get('version')=='7.2.0',[ver.get('canonical_skill_version'),osj.get('canonical_skill_version'),bm.get('bound_skill_version'),m.get('version')])
...
add('A05_skill_version_md_current','Current release: v7.2.0' in T(SK/'VERSION.md'))
add('A06_current_cert_harness_is_v72',m.get('certification_harness')=='tests/skill_certification/verify_skill_v7_2.py')
add('A07_current_redteam_is_v72','tests/skill_certification/red_team_skill_v7_2.py' in m.get('certification_harnesses',[]))
...
add('A11_global_authority_hashes_exact',gh.get('version')=='7.2.0' and not gbad,{'version':gh.get('version'),'bad':gbad[:10]})
ab=J(SK/'AUTHORITY_BINDING_CHECK.json');bbad=[]
for rel,h in ab.get('files',{}).items():
    p=SK/rel
    if not p.exists() or sha(p)!=h:bbad.append(rel)
add('A12_authority_binding_current_exact',ab.get('skill_version')=='7.2.0' and not bbad,{'version':ab.get('skill_version'),'bad':bbad[:10]})
...
sm=T(ROOT/'Rashad/Skill/MANIFEST.md'); add('P09_skill_manifest_current_v72','# Rashad Proposal OS v7.2.0 — File Manifest' in sm and '# Rashad Proposal OS v7.1.0' not in sm)
...
out={'suite':'Rashad OS v7.2 Final Package Adversarial Red Team','status':'PASS' if all(x['status']=='PASS' for x in results) else 'FAIL','blocked':sum(x['status']=='PASS' for x in results),'total':len(results),'attacks':results}
(QA/'Certification/FINAL_PACKAGE_RED_TEAM_V7_2_RESULTS.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
