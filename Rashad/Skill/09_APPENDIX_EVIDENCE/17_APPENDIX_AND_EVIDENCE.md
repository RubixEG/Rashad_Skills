...
DO NOT APPLY TO:
Using historical proposal decks as current client evidence; inventing CVs/cases/certs; treating templated `data.py` cases as verified evidence.
...
The portable core contains **no appendix PDFs/PPTX**. Runtime evidence is supplied through the external Appendix Library contract (classification: reusable Rubix evidence; **not** current RFP authority).

| Logical source class | Role |
|---|---|
| `TEAM_CV_LIBRARY` | Expanded CV evidence |
| `CASE_STUDY_LIBRARY` | Project/case evidence |
| `CORPORATE_DOCUMENT_LIBRARY` | Company registrations, certificates and formal evidence |
| `COMPANY_PROFILE_KNOWLEDGE` | Approved reusable Rubix self-description; never current-RFP fact authority |
...
1. **CV role conflicts** may exist across external CV/evidence sources for some people (see `EVIDENCE/TEAM_INDEX.md`); resolve against the current external evidence package before use.
2. **Arwa Ajarem** appears in Arabic and English — deduplicate.
3. Source `data.py` claims 26 cases with templated content for many cases — **templated content is not evidence**.
...
Corporate-document items are indexed in `EVIDENCE/CERTIFICATION_INDEX.md` and externalized from the core. All default to `VALIDITY_REVIEW_REQUIRED` until bid-date review; presence in an external library never proves current validity.

## Company profile (reusable narrative pages)

Reusable company-profile knowledge has been normalized into `11_RUBIX_FIRM_KNOWLEDGE/RUBIX_COMPANY_KNOWLEDGE_2026.md`. It is Rubix self-description knowledge — still not current RFP truth and time-sensitive claims require verification.
...
| `runtime\appendix\appendix_engine.json` | Runtime appendix engine contract |
...
| `10_PROVENANCE/PHASE_2_3_REPORT.md` | Appendix MISSING not invented |
