...
add('A01_skill_version_split_brain_blocked',ver.get('canonical_skill_version')==osj.get('canonical_skill_version')==bm.get('bound_skill_version')==m.get('version')=='7.3.0',[ver.get('canonical_skill_version'),osj.get('canonical_skill_version'),bm.get('bound_skill_version'),m.get('version')])
add('A02_brain_version_split_brain_blocked',ver.get('brain_runtime_version')==osj.get('brain_runtime_version')==bm.get('brain_version')=='3.5.0')
add('A03_artifact_version_split_brain_blocked',bm.get('artifact_brain_version')==m.get('artifact_brain_version')==art.get('artifact_brain_version')=='4.0.0')
add('A04_qa_version_current',ver.get('qa_runtime_version')==osj.get('qa_runtime_version')=='4.4')
add('A05_skill_version_md_current','Current release: v7.3.0' in T(SK/'VERSION.md'))
add('A06_current_cert_harness_is_v73',m.get('certification_harness')=='tests/skill_certification/verify_skill_v7_3.py')
add('A07_current_redteam_is_v73','tests/skill_certification/red_team_skill_v7_3.py' in m.get('certification_harnesses',[]))
...
add('A11_global_authority_hashes_exact',gh.get('version')=='7.3.0' and not gbad,{'version':gh.get('version'),'bad':gbad[:10]})
ab=J(SK/'AUTHORITY_BINDING_CHECK.json');bbad=[]
for rel,h in ab.get('files',{}).items():
    p=SK/rel
    if not p.exists() or sha(p)!=h:bbad.append(rel)
add('A12_authority_binding_current_exact',ab.get('skill_version')=='7.3.0' and not bbad,{'version':ab.get('skill_version'),'bad':bbad[:10]})
...
ac=T(B/'runtime/brain/artifact_council_runtime.py');ag=T(B/'runtime/brain/artifact_gate.py');ao=T(B/'runtime/brain/actual_output_qa.py');dg=T(B/'runtime/brain/delivery_gate.py');pi=T(B/'runtime/brain/product_inspector.py'); pg=T(B/'runtime/brain/product_geometry.py')
...
add('C12_generic_equal_cards_blocked','EQUAL_CARD_GRID_OVERUSE' in pg)
add('C13_shape_only_artifact_collapse_blocked','SHAPE_ONLY_ANALYTICAL_DECK_OVERUSE' in pg)
...
add('C16_image_led_logo_only_blocked','IMAGE_LED_DECLARED_BUT_IMAGES_APPEAR_LOGO_ONLY' in eh and 'IMAGE_LED_WITHOUT_SUBSTANTIVE_IMAGE' in pg)
...
fv=T(QA/'FINAL_VERIFY.py');add('P01_final_verify_v721_current','Final Verification v7.3' in fv and all(x in fv for x in ['verify_skill_v7_3.py','final_package_red_team_v7_3.py','handoff_lock_certification_v7_2_1.py','incident_p0_wrong_artifact_handoff_20260817.py']))
...
add('P07_current_docs_v73_v35', 'v7.3' in docs and 'v3.5' in docs and 'Canonical Rashad Layer v7.1' not in docs and 'Bound Skill: Rashad Proposal OS v7.1.0' not in docs)
...
sm=T(ROOT/'Rashad/Skill/MANIFEST.md'); add('P09_skill_manifest_current_v73','# Rashad Proposal OS v7.3.0 — File Manifest' in sm and '# Rashad Proposal OS v7.1.0' not in sm)
...
rem=J(QA/'Certification/REMEDIATION_MATRIX_V7_3.json') if (QA/'Certification/REMEDIATION_MATRIX_V7_3.json').exists() else {}
add('V73_remediation_matrix_zero_open',rem.get('status')=='PASS' and rem.get('open_count',rem.get('open'))==0 and rem.get('open_p0_p1')==0,rem)
eng=J(QA/'Certification/ENGAGEMENT_ARTIFACT_ACCEPTANCE_V7_3.json') if (QA/'Certification/ENGAGEMENT_ARTIFACT_ACCEPTANCE_V7_3.json').exists() else {}; er=eng.get('artifacts',eng.get('results',[]))
add('V73_actual_engagement_acceptance_bound',eng.get('status')=='PASS' and len(er)>=2 and all(x.get('sha256') and x.get('acceptance_status')=='BLOCKED' for x in er),er)
vp=J(QA/'Certification/VISUAL_PRODUCTION_V7_3_RESULTS.json') if (QA/'Certification/VISUAL_PRODUCTION_V7_3_RESULTS.json').exists() else {}; add('V73_visual_production_pass',vp.get('status')=='PASS',vp.get('status'))
gd=J(QA/'Certification/GOVERNED_PRODUCTION_DELIVERY_V7_3.json') if (QA/'Certification/GOVERNED_PRODUCTION_DELIVERY_V7_3.json').exists() else {}; add('V73_governed_positive_path_pass',gd.get('status')=='PASS',gd.get('status'))

out={'suite':'Rashad OS v7.3 Final Package Adversarial Red Team','status':'PASS' if all(x['status']=='PASS' for x in results) else 'FAIL','blocked':sum(x['status']=='PASS' for x in results),'total':len(results),'attacks':results}
(QA/'Certification/FINAL_PACKAGE_RED_TEAM_V7_3_RESULTS.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
