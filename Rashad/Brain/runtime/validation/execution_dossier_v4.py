...
import sys
_BRAIN=Path(__file__).resolve().parents[1]; sys.path.insert(0,str(_BRAIN)); from brain.quality_floors_v7_3 import get as quality_floor
...
def validate_page_dossier_v4(base,critical=True):
    ...
    if score<float(quality_floor('artifact_truth_min',90)) or at.get('status') not in ('PASS','PROVEN'): errs.append({'kind':'artifact_truth_below_floor','score':score,'floor':90})
    ...
    if critical and len(cands)<int(quality_floor('min_actual_render_candidates_critical',3)): errs.append({'kind':'insufficient_render_search','minimum':int(quality_floor('min_actual_render_candidates_critical',3)),'actual':len(cands)})
    hashes=[]
    for c in cands:
    ...
    if critical and len(set(hashes))<int(quality_floor('min_actual_render_candidates_critical',3)): errs.append({'kind':'render_candidates_not_distinct_by_bytes','distinct':len(set(hashes)),'required':int(quality_floor('min_actual_render_candidates_critical',3))})
    ...
    if critical and effective_min<float(quality_floor('min_pairwise_structural_divergence_critical',.12)): errs.append({'kind':'structural_divergence_below_critical_floor','actual':effective_min,'floor':float(quality_floor('min_pairwise_structural_divergence_critical',.12))})
    if critical and effective_avg<float(quality_floor('target_pairwise_structural_divergence',.18)): errs.append({'kind':'structural_divergence_below_target','actual':effective_avg,'target':float(quality_floor('target_pairwise_structural_divergence',.18))})
    # CEQS: independent >=90
    ce=objs['ceqs.json']; ceqs=float(ce.get('ceqs',ce.get('score',0)) or 0); measured['ceqs']=ceqs
    if ceqs<float(quality_floor('ceqs_min',90)) or ce.get('status') not in ('PASS','PROVEN'): errs.append({'kind':'ceqs_below_floor','score':ceqs,'floor':90})
