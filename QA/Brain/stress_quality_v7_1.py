...
OUT=ROOT/'QA/Certification'; random.seed(7102026)
...
def main():
    ...
    out={'suite':'V7.1 Artifact Stress Quality','status':'PASS' if crashes==0 and unsafe_escaped==0 and valid_failed==0 else 'FAIL','page_runs':page_runs,'deck_runs':deck_runs,'crashes':crashes,'unsafe_escaped':unsafe_escaped,'valid_failed':valid_failed}
    (OUT/'V7_1_STRESS_QUALITY_RESULTS.json').write_text(json.dumps(out,indent=2),encoding='utf8'); print(json.dumps(out,indent=2)); return 0 if out['status']=='PASS' else 2
