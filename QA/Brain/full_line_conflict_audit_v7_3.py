...
def main():
    ...
    expected={'skill':'7.3.0','brain':'3.5.0','artifact':'4.0.0','qa':'4.4','qabrain':'1.4.0'}
    ...
    if m.get('certification_harness')!='tests/skill_certification/verify_skill_v7_3.py': findings.append({'severity':'P0','kind':'STALE_CURRENT_CERT_HARNESS','value':m.get('certification_harness')})
    if 'tests/skill_certification/red_team_skill_v7_3.py' not in m.get('certification_harnesses',[]): findings.append({'severity':'P0','kind':'STALE_CURRENT_REDTEAM_HARNESS'})
    ...
    ag=(BRAIN/'runtime/brain/artifact_gate.py').read_text(); ep=(BRAIN/'runtime/brain/execution_proof.py').read_text(); ao=(BRAIN/'runtime/brain/actual_output_qa.py').read_text(); dg=(BRAIN/'runtime/brain/delivery_gate.py').read_text(); pi=(BRAIN/'runtime/brain/product_inspector.py').read_text(); pg=(BRAIN/'runtime/brain/product_geometry.py').read_text()
    for token,kind,src in [
      ('BRAIN_EXECUTION_PROOF_REQUIRED','BRAIN_PROOF_GATE_MISSING',ag),('ARTIFACT_COUNCIL_EXECUTION_REQUIRED','ARTIFACT_COUNCIL_GATE_MISSING',ag),('USER_VISIBLE_MASTER_MUST_BE_PRODUCTION_PAGE_RENDER','PRODUCTION_RENDER_GATE_MISSING',ag),('ACTUAL_PIXEL_QA_REQUIRED_FOR_USER_VISIBLE','PIXEL_QA_GATE_MISSING',ag),('QA_REPAIR_LOOP_NOT_CLOSED','REPAIR_GATE_MISSING',ag),('validate_brain_execution_proof','PROOF_RECOMPUTE_MISSING',ep),('PIXEL_REVIEW_HASH_BINDING_MISMATCH','PIXEL_HASH_BINDING_MISSING',ao),('BLOCK_DELIVERY','EXACT_FILE_DELIVERY_GATE_MISSING',dg),('DELIVERED_SLIDE_COUNT_MISMATCH_DOSSIER_PAGES','EXACT_HANDOFF_COUNT_BINDING_MISSING',(BRAIN/'runtime/brain/exact_handoff.py').read_text(encoding='utf-8')),('IMAGE_LED_DECLARED_BUT_IMAGES_APPEAR_LOGO_ONLY','IMAGE_TRUTH_HANDOFF_LOCK_MISSING',(BRAIN/'runtime/brain/exact_handoff.py').read_text(encoding='utf-8')),('EQUAL_CARD_GRID_OVERUSE','CARD_GRID_BLOCK_MISSING',pg),('SHAPE_ONLY_ANALYTICAL_DECK_OVERUSE','SHAPE_ONLY_BLOCK_MISSING',pg)]:
        if token not in src: findings.append({'severity':'P0','kind':kind})
    # Current-facing documentation must describe the v7.3/v3.5 route, never present v7.1/v3.2 as current.
    ...
    # Current v7.3 execution/certification must not depend on v7.1 test harnesses.
    ...
    for fp in current_exec:
        tx=fp.read_text(encoding='utf-8',errors='ignore') if fp.exists() else ''
        if re.search(r'(?:from|import|Brain/)[^\n]*v7_1',tx,re.I): findings.append({'severity':'P0','kind':'CURRENT_V73_DEPENDS_ON_UNROUTED_LEGACY_EXECUTION_SCRIPT','file':str(fp.relative_to(ROOT))})
    ...
    for idx in [ROOT/'Rashad/Certification/CURRENT_CERTIFICATION_INDEX.json',ROOT/'QA/Certification/CURRENT_CERTIFICATION_INDEX.json']:
        if idx.exists():
            x=J(idx)
            if x.get('canonical_skill_version') not in (None,'7.3.0') or str(x.get('status','')).startswith('CURRENT_V7_1'):
    ...
    out={'suite':'Rashad v7.3 Full Line Conflict Audit','status':'PASS' if not p0 and not p1 else 'FAIL','stats':stats,'manifest_authorities':len(globals),'p0_count':len(p0),'p1_count':len(p1),'findings':findings}
    dest=QA/'Certification/FULL_LINE_CONFLICT_AUDIT_V7_3.json'; dest.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
